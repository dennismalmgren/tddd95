...
typedef std::vector<std::vector<std::pair<int, long long> > > vvii;

const long long INF = 2000000000000;

void bellmann_ford(vvii &edges, int source,  vll &dist, std::vector<bool> &n_cycle) {
...
int main() {
	...
	cin >> n;
	if (n == -1) break;
	vvii e(n + 1, std::vector<std::pair<int, long long> >());
